// Geri Hub · webhook da Digital Manager Guru (vendas e assinaturas) → hub.orders / hub.order_deductions / hub.subscriptions
// Configurar na Guru: Configurações → Webhooks, todos os status, URL:
//   https://<projeto>.supabase.co/functions/v1/hub-guru-webhook?token=<HUB_GURU_WEBHOOK_SECRET>
// A Guru não assina o payload; o segredo na URL é a validação.
import { errMsg, insertMany, json, markIntegration, productIdFor, round2, secret, sql, upsert, upsertCustomer } from "../_shared/hub.ts";

type Any = Record<string, any>;

const statusMap: Record<string, string> = {
  approved: "approved", paid: "approved", completed: "approved", aprovada: "approved", paga: "approved",
  pending: "pending", waiting_payment: "pending", billet_printed: "pending", analysis: "pending", authorized: "pending", pendente: "pending",
  refunded: "refunded", partially_refunded: "partially_refunded", reembolsada: "refunded", estornada: "refunded",
  chargeback: "chargeback", dispute: "chargeback",
  canceled: "canceled", cancelled: "canceled", refused: "canceled", abandoned: "canceled", blocked: "canceled", cancelada: "canceled",
...
async function handleTransaction(t: Any) {
  ...
  await sql`delete from hub.order_deductions where order_id = ${order.id} and source = 'guru'`;
  const ded: Any[] = [];
  ...
  if (interest > 0) ded.push({ order_id: order.id, kind: "fee_installment", amount: round2(interest), occurred_at: soldAt, source: "guru", note: null });
  for (const a of t.affiliations ?? []) if (Number(a.value) > 0) ded.push({ order_id: order.id, kind: "commission_affiliate", amount: round2(Number(a.value)), occurred_at: soldAt, source: "guru", note: a.name ?? null });
  if (status === "refunded" || status === "chargeback" || status === "partially_refunded") {
    ded.push({ order_id: order.id, kind: status === "chargeback" ? "chargeback" : "refund", amount: round2(gross), occurred_at: t.dates?.updated_at ?? new Date().toISOString(), source: "guru", note: null });
  }
  await insertMany("order_deductions", ded);
  return order.id;
}

const subStatus: Record<string, string> = { active: "active", ativa: "active", trial: "trial", past_due: "past_due", pastdue: "past_due", atrasada: "past_due", canceled: "canceled", cancelled: "canceled", cancelada: "canceled", expired: "expired", expirada: "expired", paused: "paused", inactive: "expired", inativa: "expired" };
...
async function handleSubscription(s: Any) {
  ...
  const sub = await upsert("subscriptions", {
  ...
  if (s.last_transaction?.id) {
    ...
    await sql`update hub.orders set subscription_id = ${sub.id} where id = ${orderId}`;
  }
  return sub.id;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await markIntegration("guru", null);
    return json({ ok: true, kind, id });
  } catch (e) {
    const msg = errMsg(e);
    await markIntegration("guru", msg);
    console.error("hub-guru-webhook", msg);
